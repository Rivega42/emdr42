...
export default function AdminSettingsPage() {
  ...
  const inputClass = "w-full px-4 py-3 bg-surface border border-line rounded-md text-ink placeholder-gray-400 focus:outline-none focus:border-gray-900 transition-colors";

  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8"><h1 className="text-4xl font-bold text-ink mb-2">Platform Settings</h1><p className="text-ink-muted">Configure AI providers, protocols, and notifications</p></div>
      {error && <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 text-red-600 text-sm">{error}</div>}
      <div className="space-y-8">
        {SECTIONS.map((section) => (
          <div key={section.category} className="bg-surface border border-line rounded-lg p-6">
            <h2 className="text-2xl font-bold text-ink mb-6">{section.title}</h2>
            <div className="space-y-5">
              {section.fields.map((field) => (
                <div key={field.key}>
                  <label className="block text-ink-muted text-sm mb-1">{field.label}</label>
                  {field.description && <p className="text-ink-muted text-xs mb-2">{field.description}</p>}
