...
describe('formatTime (mm:ss таймер сессии)', () => {
  ...
  it('округляет дробные секунды (sec=Math.round)', () => {
    expect(formatTime(59.4)).toBe('00:59');
    // ⚠️ known edge: minutes использует floor, seconds — round. На границе
    // (59.6 сек) даёт '00:60', а не '01:00'. Не критично для UI, фиксируется
    // если найдётся под отдельным issue.
    expect(formatTime(59.6)).toBe('00:60');
  });
});
