...
> **Чтобы ИИ-диалог реально работал** — задайте `ANTHROPIC_API_KEY` (или
> `OPENAI_API_KEY`) в `.env` оркестратора и перезапустите его. Без ключа диалог
> недоступен, и пациент видит баннер «ИИ-ассистент недоступен» (BLS и эмоции
> при этом работают). Распознавание эмоций ключей НЕ требует — это клиентская
> инференция face-api.js, веса self-hosted в `public/models` (раньше CDN-fetch
> блокировался прод-CSP — баг исправлен). Подробный статус механик:
> [MECHANICS_STATUS.md](MECHANICS_STATUS.md).
